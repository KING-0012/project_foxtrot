...
def _party_attacks()->List[str]:
    ...
    for member_name in party_comp.members:
        ...
        if not tp.alive:continue  # player already killed this target this round
        atk=getattr(nc,"atk",6)+random.randint(-1,3)
        dmg=max(0,atk-tc.defense)
        tc.hp-=dmg
        if tc.hp<=0 and tp.alive:
            tp.alive=False
...
def combat_flee()->str:
    ...
    for e in _living_enemies():
        ...
        if ps.hp<=0:
            # _resolve_defeat calls _end_combat internally — don't call again after
            lines.append(_resolve_defeat())
            return "\n".join(lines)
    return "\n".join(lines)  # still in combat
